...
## Current implementation status check — 2026-05-14

Implemented in the current SEKER + Thoth ingest path and retained here for regression validation:
- Sequencing: logs are collected before persistence/security/device collectors that may use PowerShell/WMI/CIM.
- Host/session enrichment: `host/identity.json` includes boot time, uptime, environment/session context, username/domain/workgroup cleanup, source/confidence, and partial-field warnings.
- Richer process detail: `processes/process-details.csv` is collected after log capture via `Get-CimInstance Win32_Process`, including PID, PPID, name, executable path, command line, and owner where readable; `tasklist /fo csv /v` remains the fallback.
- Security posture: firewall, Defender, `WinDefend` fallback, and security-tool hints are collected and normalized for Thoth.
- Installed-program inventory: uninstall registry hives are collected into `software/installed-programs.json` with source metadata and install-date reliability labels.
- Device/removable-media inventory: volumes, PnP summaries, current USB devices, and source-backed previous USB evidence are collected and normalized.
- Wi-Fi/Bluetooth/context: Wi-Fi interface/profile metadata, Bluetooth PnP context, and Thoth virtual/container/VM/VPN adapter hints are present.

Pre-push cleanup completed:
- `seker --help` no longer exposes `--case-id` or `--dry-run`.
- `batch_id` is generated by default when not supplied.
- Thoth ingest no longer requires SEKER `case_id`; `bundle_id` is stored/displayed as Collection ID.

Deferred:
- User-space file triage remains SEKER v2.0.
- Rich USB first/last-seen timeline reconstruction remains a later enrichment pass.
