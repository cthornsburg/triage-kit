...
The current Thoth web UI is not macOS-specific. It is a local Go HTTP service backed by SQLite and filesystem storage. The Thoth 0.1 preview package includes macOS and Linux binaries, with Linux amd64 as the preferred VM validation target for student and contributor testing.

For Thoth 0.1, do not distribute a baseline VM image. Use the portable app package and document how to run it inside a normal Linux VM. See `docs/thoth-linux-vm-setup.md`.
